...
from deepgta.nn.simulation import qt_sim
from deepgta.util.mutual_info import calc_MI
...
extract_SNP_n = 100 #Only extract the first $effect_SNP_n SNPs with highest MI score. 
...
class DataSet(object):
    def __init__(self,
        ...
        if dummy_data:
            ...
            self._SNP,self._trait = qt_sim.run_sim(config)
        else:
...
def read_data_sets(train_info,
                   dummy_data = False
                   ):
    """Read the data set or generate the dummy data set, 
    Input Argus:
        train_info: a length 3 list include the input information [SNP_path,trait_path,trait_name]
                    SN_path point to a plink bed file, trait_path point to a trait text file with 
                    the first row indicating the trait name, and the trait_name represent the name
                    of extracting trait.
                                                        
        """
    validation_size = NUM_EXAMPLES_PER_EPOCH_FOR_VALIDATION
    if dummy_data:
        config = qt_sim.make_config(sample_size = NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN,SNP_n = 1000)
        def fake(config,extract_index=None):
            data_set = DataSet([],[],dummy_data = True,config = config)
            if extract_index is None:
                extract_index = sig_index(data_set.SNP,data_set.trait,extract_SNP_n)
            return DataSet(data_set.SNP[:,extract_index],data_set.trait,dummy_data = False),extract_index
        train,extract_index = fake(config)
        #Set the sample size to validation_size
        config.individual_n = NUM_EXAMPLES_PER_EPOCH_FOR_VALIDATION
        validation,_ = fake(config,extract_index)
        config.individual_n = NUM_EXAMPLES_FOR_TEST
        test,_ = fake(config,extract_index)
        
        return Datasets(train = train,validation = validation,test = test)
    SNP_path = train_info[0]
    trait_path = train_info[1]
    trait_name = train_info[2]
    train_SNP = extract_SNP(SNP_path)
    train_trait = extract_trait(trait_path,trait_name)
